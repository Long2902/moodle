define("tiny_digieramedia/ui", [
    ...
    const renderItems = (root, data, selectedUuid) => {
        ...
        target.innerHTML = items.map((item) => {
            ...
            return `<button type="button" class="tiny-digieramedia__card ${selected ? 'is-selected' : ''}"
                ...
                <span class="tiny-digieramedia__thumb"><span class="tiny-digieramedia__fileicon"
                    data-type="${escapeHtml(type)}">${typeLabel(type)}</span></span>
    ...
    const updatePreview = (root, selected) => {
        ...
        preview.innerHTML = `<div class="tiny-digieramedia__previewbox">
            ...
            </div><h4>${escapeHtml(selected.name)}</h4><dl>
    ...
    const showUploadProgress = (root, file, loaded, total, state = 'uploading') => {
        ...
        queue.innerHTML = `<div class="border rounded p-2 mb-2">
            <div class="d-flex justify-content-between gap-2">
                <strong class="text-truncate">${escapeHtml(file.name)}</strong><span>${escapeHtml(label)}</span>
            </div>
            <div class="progress mt-2" role="progressbar" aria-valuenow="${percent}"
                aria-valuemin="0" aria-valuemax="100">
                <div class="progress-bar" style="width:${percent}%"></div>
            </div>
            <div class="small text-muted mt-1">${formatBytes(loaded)} / ${formatBytes(safeTotal)}</div>
        </div>`;
    };
        ...
        const uploadFiles = async(files) => {
            ...
            try {
                for (const file of [...files]) {
                    ...
                    const media = await UploadClient.uploadFile(
                        config,
                        file,
        ...
        root.addEventListener('click', (event) => {
            ...
            if (nav) {
                ...
                root.querySelectorAll('[data-tab]').forEach((button) => {
                    button.classList.toggle('is-active', button === nav);
                });
                updatePreview(root, selected);
