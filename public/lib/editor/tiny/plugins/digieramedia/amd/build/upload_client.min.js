define("tiny_digieramedia/upload_client", ["exports", "core/ajax"], function(_exports, Ajax) {
    "use strict";
    ...
    const putFile = (file, session, onProgress) => new Promise((resolve, reject) => {
        ...
        request.addEventListener('error', () => {
            reject(new Error('Không thể kết nối Cloudflare R2. Kiểm tra CORS/mạng.'));
        });
        request.addEventListener('abort', () => reject(new Error('Upload đã bị hủy.')));
        request.send(file);
    });

    const uploadFile = async(config, file, onProgress = null, replacemediauuid = '') => {
