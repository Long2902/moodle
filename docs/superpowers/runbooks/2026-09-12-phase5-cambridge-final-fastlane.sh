...
# DIGIERA Phase 5 — CambridgePlus ribbon final fastlane.
# Runs locally on Web01, no GitHub Actions wait.
# It proves the visual RED checkpoint, runs the complete GREEN gate,
# freezes package-lock/runtime AMD bytes, materializes a product-only
# candidate from the current production canonical, then deploys both nodes.

REPO="Long2902/moodle"
DEV_BRANCH="digiera/tiptap-v1-phase5-official-ui"
PROD_BRANCH="digiera/preview-v01"
PROD_BASE="1b698d6a7b45a7200661df1305761910066fabcf"
VISUAL_RED_HEAD="a68d26cf383f717cbc6a2ca37501592863e1a2cd"

: "${PIN:?PIN must be the exact commit containing this runbook}"

SRC="/root/digiera-phase5-official-ui"
RED_DIR="/root/digiera-phase5-cambridge-visual-red"
MAT="/root/digiera-phase5-cambridge-materialized"
DEV_STAGE="/root/digiera-phase5-cambridge-devstage"
BASE_STAGE="/root/digiera-phase5-cambridge-baseline"
ROOT="/var/www/moodle/public"
WEB02="10.0.10.12"
CLIUSER="www-data"
LOG="/root/digiera-phase5-cambridge-final-$(date +%Y%m%d-%H%M%S).log"
...
echo '============================================================'
echo ' DIGIERA PHASE 5 — CAMBRIDGE RIBBON FINAL FASTLANE'
echo '============================================================'
echo "LOG=$LOG"
exec > >(tee -a "$LOG") 2>&1

# ------------------------------------------------------------------
# 1/9 — exact source/machine binding.
# ------------------------------------------------------------------
echo
echo '===== 1/9 SOURCE + HOST BINDING ====='
...
for command in git gh node npm php python3 rsync ssh scp sha256sum gzip tar find sort xargs cut systemctl runuser cmp grep; do
...
cd "$SRC"
# Keep locally installed dependencies outside Git status/clean operations.
grep -qxF '/public/local/digieranative/client/node_modules/' .git/info/exclude 2>/dev/null \
    || echo '/public/local/digieranative/client/node_modules/' >> .git/info/exclude
...
git merge-base --is-ancestor "$VISUAL_RED_HEAD" "$PIN" || fail 'Visual RED checkpoint is not an ancestor of PIN'
...
echo "PIN=$PIN"
echo "VISUAL_RED_HEAD=$VISUAL_RED_HEAD"
echo "PROD_BASE=$PROD_BASE"
echo 'SOURCE_HOST_BINDING=PASS'

# ------------------------------------------------------------------
# 2/9 — dependency lock sync + explicit visual RED evidence.
# ------------------------------------------------------------------
echo
echo '===== 2/9 DEPENDENCIES + VISUAL RED EVIDENCE ====='

CLIENT="$SRC/public/local/digieranative/client"
PACKAGE_JSON_BEFORE="$(sha256sum "$CLIENT/package.json" | cut -d' ' -f1)"

cd "$CLIENT"
npm install --prefer-offline --no-audit --no-fund >/tmp/digiera-cambridge-npm.log 2>&1 \
    || { tail -n 160 /tmp/digiera-cambridge-npm.log; fail 'npm install failed'; }

PACKAGE_JSON_AFTER="$(sha256sum package.json | cut -d' ' -f1)"
[ "$PACKAGE_JSON_AFTER" = "$PACKAGE_JSON_BEFORE" ] || fail 'npm modified package.json unexpectedly'

grep -q '"react-image-crop"' package-lock.json || fail 'package-lock was not synchronized with Cambridge dependencies'
grep -q '"lucide-react"' package-lock.json || fail 'package-lock missing lucide-react'

echo 'PACKAGE_LOCK_SYNC=PASS'

cd "$SRC"
git worktree remove --force "$RED_DIR" >/dev/null 2>&1 || true
rm -rf "$RED_DIR"
git worktree add --detach "$RED_DIR" "$VISUAL_RED_HEAD" >/dev/null
RED_CLIENT="$RED_DIR/public/local/digieranative/client"
rm -rf "$RED_CLIENT/node_modules"
ln -s "$CLIENT/node_modules" "$RED_CLIENT/node_modules"

set +e
(
    cd "$RED_CLIENT"
    npx vitest run tests/cambridge-visual-contract.test.js
) >/tmp/digiera-cambridge-visual-red.log 2>&1
VISUAL_RED_RC=$?
set -e

echo '--- visual RED tail ---'
tail -n 35 /tmp/digiera-cambridge-visual-red.log || true
[ "$VISUAL_RED_RC" -ne 0 ] || fail 'Cambridge visual RED unexpectedly passed'

git worktree remove --force "$RED_DIR" >/dev/null
rm -rf "$RED_DIR"
echo 'CAMBRIDGE_VISUAL_TDD_RED=PASS'

# ------------------------------------------------------------------
# 3/9 — focused + full GREEN gates. Production untouched.
# ------------------------------------------------------------------
echo
echo '===== 3/9 FULL LOCAL GREEN GATE ====='
...
npx vitest run \
    ...
    tests/paste-sanitization.test.js \
    tests/preview-structure.test.js \
    tests/schema.test.js

cd "$SRC"
python3 public/local/worksheetlibrary/tests/cambridge_asset_bridge_contract.py
python3 public/mod/worksheetgrader/tests/cambridge_attempt_asset_bridge_contract.py
...
for test in \
    public/local/worksheetlibrary/tests/native_legacy_contract.py \
    public/local/worksheetlibrary/tests/native_schema_contract.py \
    public/local/worksheetlibrary/tests/native_ui_contract.py \
    public/local/worksheetlibrary/tests/ui_v1_runtime_contract.py \
    public/mod/worksheetgrader/tests/native_preview_ui_contract.py; do
    [ ! -f "$test" ] || python3 "$test"
done

[ ! -f public/local/worksheetlibrary/tests/native_version_service_standalone.php ] \
    || php public/local/worksheetlibrary/tests/native_version_service_standalone.php
[ ! -f public/mod/worksheetgrader/tests/preview_vertical_slice_standalone.php ] \
    || php public/mod/worksheetgrader/tests/preview_vertical_slice_standalone.php
...
\$pdf->AddPage();
\$pdf->writeHTML('<h1>DIGIERA Cambridge Phase 5</h1><p>PDF smoke</p>');
\$bytes = \$pdf->Output('phase5-cambridge-smoke.pdf', 'S');
if (substr(\$bytes, 0, 4) !== '%PDF') { fwrite(STDERR, \"TCPDF smoke failed\\n\"); exit(1); }
echo \"TCPDF_SMOKE=PASS\\n\";
"

echo 'CAMBRIDGE_FOCUSED_GREEN=PASS'
echo 'FULL_TEST_GATE=PASS'

# ------------------------------------------------------------------
# 4/9 — deterministic AMD build, syntax, PHP lint.
# ------------------------------------------------------------------
echo
echo '===== 4/9 DETERMINISTIC BUILD + STATIC GATES ====='

cd "$CLIENT"
npm run build >/tmp/digiera-cambridge-build1.log 2>&1 \
    || { cat /tmp/digiera-cambridge-build1.log; fail 'Build #1 failed'; }
BUILD1="$(sha256sum ../amd/build/native_editor.min.js | cut -d' ' -f1)"
cp ../amd/build/native_editor.min.js /tmp/digiera-cambridge-build1.js

npm run build >/tmp/digiera-cambridge-build2.log 2>&1 \
    || { cat /tmp/digiera-cambridge-build2.log; fail 'Build #2 failed'; }
BUILD2="$(sha256sum ../amd/build/native_editor.min.js | cut -d' ' -f1)"
[ "$BUILD1" = "$BUILD2" ] || fail 'Native AMD build is non-deterministic'
cmp /tmp/digiera-cambridge-build1.js ../amd/build/native_editor.min.js >/dev/null \
    || fail 'Native AMD build bytes differ'
...
find \
    ...
    | xargs -0 -n1 php -l >/tmp/digiera-cambridge-php-lint.log
...
# ------------------------------------------------------------------
# 5/9 — freeze package-lock + generated runtime bytes on dev branch.
# ------------------------------------------------------------------
echo
echo '===== 5/9 FREEZE GREEN SOURCE ====='
...
for f in "${DIRTY[@]}"; do
    case "$f" in
        ...
        public/mod/worksheetgrader/amd/build/native_attempt.min.js)
            ;;
        *) fail "Unexpected dirty file after GREEN/build: $f" ;;
    esac
done

git add public/local/digieranative/client/package-lock.json
for f in \
    public/local/digieranative/amd/build/native_editor.min.js \
    public/local/worksheetlibrary/amd/build/native_editor.min.js \
    public/local/worksheetlibrary/amd/build/browser.min.js \
...
if ! git diff --cached --quiet; then
    git commit -m 'build(phase5): freeze Cambridge ribbon local green artifacts [skip ci]' >/dev/null
fi
...
[ "$REMOTE_DEV_BEFORE" = "$PIN" ] || fail 'Development branch raced before artifact push'
...
echo 'GREEN_SOURCE_FREEZE=PASS'

# ------------------------------------------------------------------
# 6/9 — product-only canonical candidate + deterministic package.
# ------------------------------------------------------------------
echo
echo '===== 6/9 PRODUCT CANONICAL + PACKAGE ====='
...
[ "${#CHANGED[@]}" -gt 0 ] || fail 'No Cambridge product delta'
for f in "${CHANGED[@]}"; do
    case "$f" in
...
git -C "$MAT" commit -m 'feat(phase5): CambridgePlus ribbon and picture workflow final [skip ci]' >/dev/null
...
PKG="/root/digiera-phase5-cambridge-${SHORT}.tar.gz"
PKG2="/root/digiera-phase5-cambridge-${SHORT}.verify.tar.gz"
STAGE="/root/digiera-phase5-cambridge-stage-${SHORT}"
rm -rf "$STAGE"
rm -f "$PKG" "$PKG2"
mkdir -p "$STAGE"
...
git -C "$MAT" archive --format=tar "$CANON_SHA" \
    public/local/digieranative \
    public/local/worksheetlibrary \
    public/mod/worksheetgrader | gzip -n -9 >"$PKG"
git -C "$MAT" archive --format=tar "$CANON_SHA" \
    public/local/digieranative \
    public/local/worksheetlibrary \
    public/mod/worksheetgrader | gzip -n -9 >"$PKG2"
...
echo 'IMMUTABLE_PACKAGE=PASS'

# ------------------------------------------------------------------
# 7/9 — exact production baseline, backup, Web02 stage.
# ------------------------------------------------------------------
echo
echo '===== 7/9 PRODUCTION BASELINE + BACKUP ====='
...
git -C "$MAT" archive --format=tar "$PROD_BASE" \
    public/local/digieranative \
    public/local/worksheetlibrary \
    public/mod/worksheetgrader | tar -x -C "$BASE_STAGE"
...
B1="/root/digiera-p5-cambridge-pre-${STAMP}"
B2="/root/digiera-p5-cambridge-pre-${STAMP}"
...
REMOTE_STAGE="/root/digiera-phase5-cambridge-stage-${SHORT}"
...
# ------------------------------------------------------------------
# 8/9 — two-node package-bound deploy + runtime verification.
# ------------------------------------------------------------------
echo
echo '===== 8/9 TWO-NODE DEPLOY ====='
MUTATED=1
...
# ------------------------------------------------------------------
# 9/9 — canonical fast-forward + state.
# ------------------------------------------------------------------
echo
echo '===== 9/9 CANONICAL + FINAL STATE ====='
...
STATE="/root/digiera-phase5-cambridge-final-state.env"
cat >"$STATE" <<EOF
...
echo
echo '============================================================'
echo ' PHASE 5 CAMBRIDGE RIBBON FINAL FASTLANE RESULT'
echo '============================================================'
echo 'CAMBRIDGE_VISUAL_TDD_RED=PASS'
echo 'CAMBRIDGE_FOCUSED_GREEN=PASS'
echo 'TEACHER_IMAGE_ADAPTER=PASS'
echo 'STUDENT_IMAGE_ADAPTER=PASS'
echo 'FULL_TEST_GATE=PASS'
echo 'TCPDF_SMOKE=PASS'
echo 'DETERMINISTIC_BUILD=PASS'
echo 'PHP_LINT=PASS'
...
echo 'DB_UPGRADE_REQUIRED=NO'
echo 'PHASE5_CAMBRIDGE_DEPLOY=PASS'
